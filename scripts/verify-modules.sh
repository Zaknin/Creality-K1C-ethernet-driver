...
EXPECTED_VERMAGIC="4.4.94 SMP preempt mod_unload MIPS32_R2 32BIT"
...
for module in $(expected_modules); do
  ...
  if command -v modinfo >/dev/null 2>&1; then
    ...
    if grep '^vermagic:' "$REPORT_DIR/$module.modinfo.txt" >/dev/null 2>&1; then
      ...
      grep '^vermagic:' "$REPORT_DIR/$module.modinfo.txt" | grep "$EXPECTED_VERMAGIC" >/dev/null 2>&1 || die "$module vermagic does not match expected K1C ABI"
    fi
  fi
